import asyncio
from agents import (
    Agent, GuardrailFunctionOutput, HandoffOutputItem, ItemHelpers,
    MessageOutputItem, RunContextWrapper, Runner, TResponseInputItem,
    ToolCallItem, ToolCallOutputItem, WebSearchTool, function_tool, input_guardrail
)
...
city_info_agent = Agent[UserInfoContext](
    ...
    You are a city info agent. If you are speaking to a customer,
    you were probably transferred to from the triage agent.
    Use the following routine to support the customer
    # Routine
    1. Ask for the city name
    2. Use the web search tool to get information about restaurants in the city.
    3. Use the weather tool to get the weather in the city.
    4. If the customer asks a question that is not related to the routine, transfer
...
flight_finder_agent = Agent[UserInfoContext](
    ...
    instructions=f"""
    {RECOMMENDED_PROMPT_PREFIX}
    You are a flight finder agent. If you are speaking to a customer,
    you were probably transferred to from the triage agent.
    Use the following routine to support the customer
    1. Ask for the city name
    2. Use the web search tool to get information about flights to the city.
    3. If the customer asks a question that is not related to the routine, transfer
    back to the triage agent.
    """,
    model="gpt-4o-mini",
    tools=[WebSearchTool()]
...
router_agent = Agent[UserInfoContext](
    ...
    handoff_description="A triage agent that can help in delegating a customer's request to the appropirate agent.",
    instructions=f"""
    {RECOMMENDED_PROMPT_PREFIX}
    You are a helpful routing agent. You can use your tools to delegate
